...
def load_config(path: Path | None = None) -> Config:
    ...
    if path is not None:
        ...
        _reject_unwaived_error_downgrades(rules, waivers)
        sources.append(str(path))
    return Config(thresholds, rules, waivers, tuple(sources))


def _reject_unwaived_error_downgrades(
    rules: dict[str, dict[str, Any]], waivers: tuple[Waiver, ...]
) -> None:
    """An ERROR rule may not be set to warn or off without a matching waiver."""
    ...
    for rule_id, options in rules.items():
        ...
        if requested not in {"warn", "off"}:
            continue
        if defaults.get(rule_id) != Severity.ERROR:
            continue
        if rule_id not in waived:
            raise ValueError(f"lowering {rule_id} from ERROR requires a waiver")
